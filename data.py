...
openai.api_key = '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'  # Setting your API key
...
for g1 in range(len(gfiles)):  # Iterating through every document
    ...
    f = open(f"embs{g1}.csv", "w", encoding="utf8")  # Creating a csv file for storing the embeddings for your ChatBot
...
with open(f"{gfiles[g1]}", 'r', encoding="utf8") as file:  # Storing the document contents
    content += file.read()
    content += "\n\n"
...
def get_embedding(text,
                  model="text-embedding-ada-002"):  # Defining the function that creates the embeddings needed for the Chatbot to function (It can't form answers from plain text)
    text = text.replace("\n", " ")
...
df["embedding"] = df.combined.apply(lambda x: get_embedding(
    x))  # Adding and filling the 'embedding' collumn which contains the embeddings created from your text chunks
